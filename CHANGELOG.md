...
- **`consultar()->eventos()` exigia um campo que a rota não devolve — nenhuma consulta de evento existente podia ter sucesso.** O método pedia `eventoXmlGZipB64` no topo da resposta, que é o que `EventosPostResponseSucesso` (SefinNacional-swagger.json) declara e o que o próprio swagger aponta como resposta 200 do `GET /nfse/{chave}/eventos/{tipoEvento}/{nSequencial}`. O `SefinNacional_1.6.0` devolve outra coisa: uma lista `eventos[]`, com o XML em `eventos[].arquivoXml` e uma segunda camada de base64 sobre o gzip. Só o caminho da ausência funcionava — evento inexistente responde 404 e vira `EVENT_NOT_FOUND`; evento presente responde 200, não trazia o campo exigido e virava `IndeterminateResultException`. Quanto mais bem-sucedida a operação no SEFIN, pior a leitura do SDK: a reconciliação de um cancelamento registrado concluía "não foi possível confirmar", e uma nota cancelada por análise fiscal seguia autorizada no banco do consumidor. A suíte não via nada porque os fixtures foram construídos a partir do schema do swagger, não de tráfego real. As duas formas passam a ser aceitas — `eventos[]` primeiro, `eventoXmlGZipB64` de topo depois —, porque `storage/prefeituras.json` tem prefeituras com implementação própria que podem cumprir o contrato declarado. `EventsResponse` ganha `eventos`, uma lista de `EventoConsultado` com `chaveAcesso`, `tipoEvento`, `numeroPedidoRegistroEvento`, `dataHoraRecebimento`, `xml` e `parseError`; `xml` continua sendo o XML do primeiro item. Um evento ilegível não derruba a lista: os campos afetados vêm `null` e `parseError` diz o que faltou, como o `DocumentoFiscal` já fazia no lote de distribuição.

- **`IndeterminateResultException` descartava a resposta que a motivou — a recusa era correta e indiagnosticável.** O `statusCode` chegava às fábricas, entrava na mensagem formatada e não virava dado; o corpo se perdia por completo. Do lado do consumidor a exceção rendia duas linhas de log sem nada sobre o que a SEFIN mandou, e diagnosticar o defeito acima exigiu escrever um script, subir no servidor de produção e refazer a chamada com o certificado — por fora do SDK. A exceção passa a carregar `statusCode`, `body` (truncado em 8 KiB) e `raw` (o JSON decodificado, sem truncar), preenchidos com o que cada ponto de falha tem em escopo. `NfseFailed` ganha `throwable`, para que o listener alcance a mesma evidência sem depender do `catch` do chamador — antes ele recebia `$throwable->getMessage()`, a mesma string da exceção.

### Changed

- **`GzipCompressor::decompressB64()` descasca uma segunda camada de base64 quando ela existe.** Condicionado ao magic gzip e limitado a duas camadas: se o primeiro decode não revelar `1f 8b`, tenta mais um e fica com o resultado apenas se ele for gzip. É o formato de `eventos[].arquivoXml` no `SefinNacional_1.6.0`, e cobre o `ArquivoXml` da distribuição caso o ADN faça o mesmo — hipótese ainda não medida.

- **`ExecutesNfseRequests::executeRaw()` aceita campos obrigatórios alternativos**, declarados pela forma que a operação lê: `$requiredStrings` (texto não-vazio) e `$requiredLists` (lista não-vazia). Campo presente é o que veio **na forma exigida** — um `chaveAcesso` que chegue como lista continua sendo resposta fora do contrato, e não um campo presente que estouraria `TypeError` no construtor tipado logo adiante. Porta interna, marcada `@internal`.

- **Rejeição cujo `erro` singular vem como lista chegava ao consumidor sem código nem mensagem.** `ResponseErro` (SefinNacional-swagger.json) declara `erro` como uma `MensagemProcessamento`, mas a SEFIN também o envia como lista de mensagens — confirmado em produção com `SefinNacional_1.6.0`, numa rejeição `E0822` ("O prazo para o cancelamento da NFS-e expirou") de cancelamento. `extractErrors()` embrulhava o valor sem distinguir as duas formas, então a lista virava uma mensagem cuja única chave é o inteiro `0`; `fromArray()` procurava `codigo`/`mensagem`/`descricao`, não achava nada, e o evento `NfseRejected` saía com `UNKNOWN` e `null`. A classificação nunca esteve errada — `hasApiError()` devolvia `true`, a operação foi corretamente tratada como rejeitada —, o que se perdia era justamente o conteúdo que decide o próximo passo do consumidor. Mapa e lista passam pelo mesmo filtro de `onlyMessages()`, então as tolerâncias seguem valendo: `{"erro": "Bad Gateway"}` de proxy continua fora da classificação, e item não-mensagem no meio da lista continua sendo descartado sem levar os erros reais junto.

- **`{"erros":[{}]}` classificava como rejeição sem conteúdo, enquanto `{"erro":{}}` não.** A assimetria vinha de `extractErrors()`: o singular descartava mensagem vazia, o plural só filtrava o que não fosse array. O consumidor recebia uma rejeição definitiva sem código nem texto — indistinguível, para ele, do defeito acima. Item vazio agora sai nos dois caminhos; sem mensagem alguma o status HTTP volta a ser a informação definitiva, e ele chega por `IndeterminateResultException` (POST 5xx) ou `HttpException`, ambas com o corpo íntegro.

- **`CertificateManager` aceitava um certificado que ainda não entrou em vigência.** A única guarda era `isExpired()`, que checa apenas `validTo`; um PFX com `validFrom` no futuro — renovação emitida com antecedência, ou host com relógio adiantado — passava na construção e só falhava depois, no handshake mTLS ou na assinatura, com um erro opaco de transporte em vez de um erro de certificado claro. A construção passa a verificar também `now < validFrom` e lança a nova `CertificateNotYetValidException`. O instante de referência é injetável (`?DateTimeImmutable $now`, default `now`), o que torna a janela de vigência testável sem depender de um fixture com data futura.

- **Um XML autorizado porém corrompido descartava a chave da nota (ou o recibo do evento) e virava falha silenciosa.** `emitir()`, `cancelar()` e `substituir()` despachavam o evento de sucesso e só então descomprimiam o XML como argumento do construtor da resposta. Com `chaveAcesso`/`eventoXmlGZipB64` presente (nota autorizada, evento registrado) mas o gzip corrompido — base64 inválido, gzip inválido ou acima do teto de 50 MB —, `GzipCompressor::decompressB64()` lançava `NfseException` **depois** de `NfseEmitted`/`NfseCancelled`: o caller recebia uma exceção sem a chave de uma nota que a SEFIN já tinha gravado (a mesma perda de reconciliação que o ramo `SEM_CHAVE` existe para evitar), e o stream registrava `NfseEmitted` seguido de `NfseFailed` para a mesma chamada. Agora essas operações preservam o identificador: a mudança de estado já aconteceu, então um XML ilegível vira `xml: null` mais um alerta `XML_ILEGIVEL` cujo `complemento` carrega o motivo técnico (base64 vs. descompressão) — falha explícita, não silenciosa —, e o documento se recupera por `consultar()->nfse()`/`->eventos()`. `consultar()->nfse()` é read-only (o XML é o próprio fruto do pedido, não há identificador a preservar), então segue lançando; a correção ali foi só a ordem — descomprimir antes do evento, para um corpo corrompido disparar `NfseFailed` em vez de um `NfseQueried` falso.

- **Um `erro`/`erros` escalar de proxy/WAF virava rejeição definitiva da SEFIN — e depois `TypeError`.** `extractErrors()` — de onde `hasApiError()` e `fromApiResult()` derivam — aceitava `erro` de qualquer tipo. Um intermediário antes da SEFIN (proxy, WAF, gateway, comuns na infra BR e com JSON próprio) que responde `502 {"erro": "Bad Gateway"}` passava por `hasApiError()`, e `NfseHttpClient::request()` devolvia o corpo como rejeição processada — **antes** do `IndeterminateResultException` que o 5xx em POST dispara. Uma nota possivelmente gravada era classificada como rejeitada em definitivo, convidando a reemissão duplicada — exatamente o que os comentários em volta existem para evitar. E, mesmo antes disso, `fromArray()` recebia a string onde espera array e estourava `TypeError` sob `strict_types`, fora do contrato de exceções tipadas; o `@var array{erros?: list<MessageData>, erro?: MessageData}` da chamada só silenciava o PHPStan, nada validava a forma em runtime. Agora, no mesmo espírito de tolerância do `DistribuicaoResponse`, só array conta como mensagem: `erros` filtra itens não-mensagem (`array_filter(is_array(...))`, preservando os erros reais de uma lista mista) e um `erro` escalar sai da classificação — o 5xx cai em `IndeterminateResultException` e o caller reconcilia por `consultar()->dps()`, como o README já documenta.

- **`emitirDecisaoJudicial()` enviava uma DPS a um endpoint que recebe a NFS-e inteira — nunca funcionou.** O método reusava o corpo de `emitir()`: montava o XML com o `DpsBuilder`, assinava `infDPS`/`DPS` e só trocava a chave do payload (`xmlGZipB64`) e a rota (`decisao-judicial/nfse`). Mas `NFSeBypassPostRequest.xmlGZipB64` (SefinNacional-swagger.json) é "Documento XML da NFSe compactado no padrão GZIP", e em `TCInfNFSe` a `DPS` é apenas o **último filho** — ao lado de `nNFSe`, `nDFSe`, `cStat`, `dhProc`, `verAplic` e dos valores já apurados (`vBC`, `pAliqAplic`, `vISSQN`), com `ds:Signature` no nível `NFSe` e `Id` de 53 posições. O que saía era, portanto, um fragmento do documento contratado. Nem cabe implementar a operação neste SDK: `ambGer` (`TSAmbGeradorNFSe`) admite somente `1-Prefeitura` e `2-Sistema Nacional da NFS-e` — não há valor para contribuinte —, e `tpEmis` 2 descreve "emissão original em leiaute próprio do município com transcrição para o modelo nacional"; os campos obrigatórios do envelope são todos do gerador da nota. A suíte não via nada porque o teste afirmava apenas `isset($req['xmlGZipB64'])`, nunca o conteúdo, e reusava a fixture de sucesso de `emitir()`. O método continua na interface `EmitsNfse` e lança `NfseException` na entrada, antes de qualquer evento ou requisição, com a explicação e as citações do schema na mensagem — falha em milissegundos, em vez de um round-trip terminado em rejeição genérica da SEFIN que nenhum ajuste de DPS resolve.

- **`consultar()->nfse()` aceitava um 200 sem `nfseXmlGZipB64` como sucesso — com o único fruto da consulta em branco.** `NFSeGetResponseSucesso` (SefinNacional-swagger.json) declara `chaveAcesso` e `nfseXmlGZipB64` required no 200, mas o parse só perguntava pelo envelope de erro: um 2xx fora do contrato virava consulta "bem-sucedida" com `xml` e `chave` nulos, `NfseQueried` disparado, e quem encadeia `danfse()->toPdf($response->xml)` recebia null onde o contrato promete string. A correção da 2.x cobriu só o status não-2xx; a mesma régua de `eventos()` e `dps()` agora vale aqui: 2xx sem o campo lança `IndeterminateResultException`, com `NfseFailed` no lugar de `NfseQueried`.

- **`distribuicao()` morria num `TypeError` quando `StatusProcessamento`, `Alertas` ou `Erros` vinham com o tipo errado.** A classe já tinha assumido a postura de tolerância a resposta fora do contrato — o branch `INVALID_RESPONSE` diz literalmente "Campo StatusProcessamento ausente ou inválido", `LoteDFe` filtra item não-array, `VersaoAplicativo`/`DataHoraProcessamento` conferem `is_string` — mas deixou o campo-pivô e as duas listas de mensagens desprotegidos: um `StatusProcessamento` numérico estourava no `tryFrom` sob `strict_types` antes de alcançar o branch escrito para ele, e `Alertas`/`Erros` fora do formato derrubavam o `fromArrayList`. Como no endurecimento do `DocumentoFiscal`, isto é tolerância deliberada, não correção de contrato: o `ADN-Contribuinte-swagger.json` declara o status como enum de string e as listas como arrays — um ADN em conformidade nunca alcança estas guardas. Status de tipo errado agora cai no `INVALID_RESPONSE`; item não-array das listas sai delas, como o `LoteDFe` já fazia.

- **`GzipCompressor::decompressB64()` descomprimia resposta do servidor sem teto (CWE-409).** Gzip alcança razões de expansão da ordem de 1000:1, então uma resposta de poucos MB — de um endpoint comprometido, ou via MITM em homologação com `sslVerify: false` — podia expandir GB em memória e derrubar o processo consumidor. `gzdecode()` agora recebe teto de 50 MB, folga larga para qualquer XML de NFS-e ou documento de distribuição; acima disso retorna `false` e cai no contrato de erro já existente (`Falha ao descomprimir XML.`).

- **`consultar()->dps()` aceitava um 200 sem `chaveAcesso` como sucesso — com chave nula, no exato fluxo que existe para recuperá-la.** `DpsGetResponse` (SefinNacional-swagger.json) declara `chaveAcesso` required no 200, e o mecanismo para exigi-la — o `requiredField` de `executeRaw()`, criado na 2.7.0 — já era usado por `eventos()`, mas `dps()` chamava sem ele. Um 2xx fora do contrato virava `NfseResponse(sucesso: true, chave: null)`, e o exemplo de reconciliação do README manda ler `$lookup->sucesso` como "a nota FOI emitida: salvar chave" — o consumidor encerrava a reconciliação pós-timeout convencido da emissão e sem identificador algum da nota. Agora `dps()` passa `'chaveAcesso'` e o 2xx sem ela vira `IndeterminateResultException`, com `NfseFailed` no lugar de `NfseQueried`; o 404 (`DPS_NOT_FOUND`) segue intacto, porque o pipeline o devolve antes da checagem. Um teste fixava a chave nula como comportamento desejado e foi substituído.

- **`cancelar()` relatava sucesso — e disparava `NfseCancelled` — para respostas que não provam o registro do evento.** O parse só distinguia "tem envelope de erro" de "não tem", e duas rotas reais chegam lá sem prova alguma: um 2xx sem `eventoXmlGZipB64`, que `EventosPostResponseSucesso` (SefinNacional-swagger.json) declara obrigatório; e o 4xx de proxy/WAF com JSON próprio, que `request()` devolve ao POST para servir o resgate `SEM_CHAVE` do emitter — resgate que o cancelamento não tem. O mesmo corpo que na emissão virava `sucesso: false` virava `sucesso: true` no cancelamento, e o chamador seguia acreditando numa nota cancelada que pode nunca ter registrado o evento. A régua já era da casa desde a 2.7.0 em `consultar()->eventos()` — "classificação exige certeza, viés para indeterminado" — mas não valia justamente para o POST, que é quem altera estado. Sem rejeição estruturada e sem o recibo, agora é `IndeterminateResultException`, com `NfseFailed` no lugar de `NfseCancelled`, e o chamador reconcilia por `consultar()->eventos()`, que é o fluxo que o README já documenta.
...
- **`solicitarAnaliseFiscalCancelamento()` — o caminho que resta quando o prazo do município expirou.** Passado o prazo de cancelamento direto, a SEFIN rejeita `cancelar()` com `E0822` e a NFS-e só sai de circulação por análise fiscal: o evento `e101103` (`TE101103` em `tiposEventos_v1.01.xsd`), mesmo `POST /nfse/{chave}/eventos` e mesma estrutura do `e101101` — muda o texto de `xDesc`, fixado por enumeração no XSD, e o sufixo do Id do pedido. Era o único fluxo do portal nacional sem equivalente aqui. Sucesso significa **pedido registrado**, não nota cancelada: dispara o novo evento `NfseFiscalAnalysisRequested`, nunca `NfseCancelled`, porque a nota segue válida até o fisco decidir.

- **`consultar()->situacaoCancelamento()` — em que ponto está a análise fiscal, numa chamada.** Enquanto a prefeitura analisa, não há nada na nota que revele isso: `TStat` segue `100` (Gerada) do começo ao fim. O estado existe só nos eventos vinculados à chave — o pedido (101103) e seu desfecho (105104 deferido, 105105 indeferido) —, e obtê-lo exigia do consumidor três consultas à SEFIN, uma por tipo, ou percorrer o lote do ADN à mão. O método lê `GET /NFSe/{ChaveAcesso}/Eventos` do ADN e devolve o novo enum `SituacaoCancelamento` (`SemPedido`, `EmAnalise`, `Deferido`, `Indeferido`). Vence o evento de maior NSU, e não a posição no lote: o ADN não promete ordem, e um pedido novo depois de um indeferimento reabre a análise. Rejeição do ADN lança `NfseException` em vez de virar `SemPedido` — consulta que falhou não é nota sem pendência.

- **`raw` em `NfseResponse`, `EventsResponse` e `DistribuicaoResponse` — o corpo decodificado deixa de ser descartado.** O consumidor não tinha caminho público para ler a resposta que a normalização não interpretou: o corpo só escapava por exceção (`HttpException::getResponseBody()`, `IndeterminateResultException`), e na rejeição morria dentro de `NfseHttpClient::request()`, que devolve apenas o array já decodificado. Diagnosticar uma rejeição fora do contrato exigia reflexão sobre classes `final readonly` em produção — foi o que o time consumidor precisou fazer para descobrir o `E0822` acima. O campo é preenchido em sucesso e em rejeição, e completa o que `FORMATO_DESCONHECIDO` só resolve para a mensagem: se a SEFIN renomear a chave de topo do envelope, `hasApiError()` devolve `false` e `raw` passa a ser a única evidência. `DanfseResponse` fica de fora — o corpo dela é o PDF binário, já exposto em `pdf`.

- `ProcessingMessage::FORMATO_DESCONHECIDO` — mensagem sem nenhuma chave conhecida deixa de chegar inteiramente nula. `codigo` recebe a constante e `complemento` carrega o item original em JSON, que `NfseRejected` expõe em `correcao`. Sem isso, um formato novo da SEFIN é indistinguível de uma resposta sem conteúdo — foi o sintoma que escondeu o `E0822` acima.

- `Enums\EventoCancelamento` — os eventos que `pedRegEvento` aceita para retirar uma NFS-e de circulação (`Cancelamento`, `SolicitacaoAnaliseFiscal`), com a tag do elemento e o `xDesc` que o XSD enumera para cada um.

- `Enums\SituacaoCancelamento` — situação da análise fiscal, deduzida dos eventos da nota.

- `Events\NfseFiscalAnalysisRequested` — pedido de análise fiscal registrado; carrega `chave`.
...
- **BREAKING (só para quem implementa as portas) — `CancelsNfse` e `ConsultsNfse` ganharam um método cada:** `solicitarAnaliseFiscalCancelamento()` e `situacaoCancelamento()`. Nenhum uso público muda; quebra apenas implementações próprias das interfaces, que precisam declarar os métodos novos. `NfseConsulter` — marcada `@internal`, construída só pelo `NfsenClient` — passou a receber a porta `DistributesNfse` como sexto parâmetro do construtor, porque a situação da análise fiscal vem do endpoint de eventos do ADN.
...
- **A descrição do Packagist deixa de esconder o modo standalone.** `composer.json` dizia "Pacote Laravel para emissão de NFSe Nacional" enquanto o README vende `forStandalone()` como caminho de primeira classe — quem procurava um SDK framework-free descartava o pacote pela descrição. Agora: "SDK PHP para emissão de NFS-e Nacional — Laravel ou standalone (sem framework)", com `standalone` e `sdk` nas keywords.
